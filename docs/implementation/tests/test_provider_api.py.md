...
FastAPI TestClient composes the real local authorizer, SQLite encrypted store and audit store in temporary directories. The fixture supplies explicit synthetic public control and a one-shot initializer returning those real services and its original credentials. Provider assertions are preserved; preinitialized fixture stores are not startup-order evidence. Fixture credentials distinguish write, resolve, generation, metadata and revoke powers. These are provider HTTP laws, not CPK or deployed transport semantics.
